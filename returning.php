	...
	<tbody>
		<?php
			...
			while($freturn = $qreturn->fetch_array()){
				...
				// Borrow date (the existing 'date' field)
				$borrow_date = strtotime($freturn['date']);  // Converting the date to a timestamp
				$due_date = strtotime('+5 days', $borrow_date); // Adding 5 days to the borrow date
				...
				if($freturn['status'] != 'Returned'){
					...
					if ($today > $due_date) {
						$diff = floor(($today - $due_date) / (60 * 60 * 24)); // Difference in days
						$penalty = $diff * 5; // ₹5 per day penalty
					}
				}
		?>
		<tr>
			...
			<td><?php echo date("m-d-Y", $due_date); ?></td> <!-- Display due date -->
			<td>
				<?php
					if($penalty > 0){
						echo '<span style="color:red;">₹' . $penalty . '</span>';
					} else {
						echo "-";
					}
				?>
			...
			<td>
				<?php 
					if($freturn['status'] == 'Returned'){
						echo '<center><button disabled class="btn btn-primary"><span class="glyphicon glyphicon-check"></span> Returned</button></center>';	
					}else{
