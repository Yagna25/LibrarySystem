...
<?php
	...
	$categories = ["Fiction", "Non-fiction", "Science", "Technology", "History", "Biography", "Children", "Fantasy"];
?>	
<html lang = "eng">
	<head>
    ...
    <input type = "number" min = "0" max = "100" name = "qty" required = "required" class = "form-control" />
</div>
								<div class = "form-group">
...
<div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
  ...
  $(document).ready(function () {
    ...
    $('.delbook_id').click(function () {
      ...
      $('#confirmMessage').text('Are you sure you want to delete this book?');
      $('#confirmModal').modal('show');
    });

    // Edit book with modal confirmation
    $('.ebook_id').click(function () {
      actionType = 'edit';
      selectedId = $(this).attr('value');
      $('#confirmMessage').text('Are you sure you want to edit this book?');
      $('#confirmModal').modal('show');
    });
    ...
    $('#confirmYes').click(function () {
      ...
      if (actionType === 'delete') {
        window.location = 'delete_book.php?book_id=' + selectedId;
      } else if (actionType === 'edit') {
        $('#show_book').show();
        $('#book_table').fadeOut();
        $('#add_book').hide();
        $('#edit_form').load('load_book.php?book_id=' + selectedId);
      }
    });
  });
</script>
